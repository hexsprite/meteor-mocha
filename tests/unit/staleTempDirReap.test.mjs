...
test('stopDaemon returns false when the tracked pid is still alive after the timeout', async () => {
  const stopped = await stopDaemon(true, {
    readPidFile: () => 999,
    isRunning: () => true,
    // No listener, so identity rests on the command line looking like ours.
    getPortHolders: () => [],
    getCommandLine: () => 'npm run test:daemon',
    killGroup: () => {},
    clearPidFile: () => {},
    waitForTerm: async () => ({ stopped: false, alivePids: [999] }),
  });
...
test('stopDaemon (port fallback path) returns false when killed pids do not clear', async () => {
  const stopped = await stopDaemon(true, {
    readPidFile: () => null,
    isRunning: () => false,
    // A LISTENER on the port; client sockets never reach this path (fo-pi0kl).
    getPortHolders: () => [4242],
    getGroupId: () => 4242,
    killPid: () => {},
    clearPidFile: () => {},
    waitForTerm: async () => ({ stopped: false, alivePids: [4242] }),
  });
  assert.equal(stopped, false);
});
